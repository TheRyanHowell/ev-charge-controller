...
export async function GET(
  ...
  return proxyGet({
    path: `/api/plugs/${plugId}`,
    detail: "Failed to fetch plug",
  });
}
...
export async function PATCH(
  ...
  return proxyRequest({ path: `/api/plugs/${plugId}`, method: "PATCH", body });
}
...
export async function DELETE(
  ...
  const { plugId } = await params;
  return proxyRequest({ path: `/api/plugs/${plugId}`, method: "DELETE" });
}
