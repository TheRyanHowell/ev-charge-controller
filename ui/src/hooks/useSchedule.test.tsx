...
describe("useSchedule", () => {
  ...
  it("does not clobber the plugs list cache when no plug is selected", async () => {
    vi.stubGlobal("fetch", vi.fn());
    const queryClient = makeQueryClient();
    const plugs = [{ id: "p1", name: "Garage Plug" }];
    queryClient.setQueryData(queryKeys.plugs.all, plugs);

    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <QueryClientProvider client={queryClient}>{children}</QueryClientProvider>
    );
    const { result } = rawRenderHook(() => useSchedule(null), { wrapper });

    // The no-plug schedule query must not share the plugs-list cache entry.
    expect(result.current.schedule).toBeNull();

    // Invalidating the plugs list (e.g. right after creating a plug) must not
    // let the disabled schedule query's queryFn overwrite the list with null.
    await act(async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.plugs.all });
    });
    expect(queryClient.getQueryData(queryKeys.plugs.all)).toEqual(plugs);
  });
