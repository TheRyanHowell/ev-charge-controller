...
func (r *ScheduleRepository) Get(ctx context.Context) (*models.Schedule, error) {
	...
	query := `SELECT id, plug_id, user_id, type, time, window_start, window_end, ready_by, two_stage, enabled FROM schedules` + wc + ` LIMIT 1`
	...
	err := r.db.QueryRowContext(ctx, query, wa...).Scan(
		...
		newNullString(&s.ReadyBy), &s.TwoStage, &s.Enabled,
	)
...
func (r *ScheduleRepository) Upsert(ctx context.Context, schedule *models.Schedule) error {
	const upsertSQL = `INSERT INTO schedules (id, plug_id, user_id, type, time, window_start, window_end, ready_by, two_stage, enabled)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	_, err := r.db.ExecContext(ctx, upsertSQL,
		...
		toNullString(schedule.ReadyBy), schedule.TwoStage,
		schedule.Enabled)
	return err
}
...
func (r *ScheduleRepository) GetByPlugID(ctx context.Context, plugID string) (*models.Schedule, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, plug_id, user_id, type, time, window_start, window_end, ready_by, two_stage, enabled FROM schedules WHERE plug_id = ?`, plugID,
...
func (r *ScheduleRepository) UpsertByPlugID(ctx context.Context, schedule *models.Schedule) error {
	const q = `INSERT INTO schedules (id, plug_id, user_id, type, time, window_start, window_end, ready_by, two_stage, enabled)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func (r *ScheduleRepository) ListAll(ctx context.Context) ([]models.Schedule, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, plug_id, user_id, type, time, window_start, window_end, ready_by, two_stage, enabled FROM schedules WHERE plug_id IS NOT NULL`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, newNullString(&s.PlugID), newNullString(&s.UserID),
			&schedType, &s.Time, newNullString(&s.WindowStart), newNullString(&s.WindowEnd),
			newNullString(&s.ReadyBy), &s.TwoStage, &s.Enabled); err != nil {
			return nil, err
		}
		s.Type = models.ScheduleType(schedType)
		out = append(out, s)
